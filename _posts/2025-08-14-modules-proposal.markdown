...
In theory, the importer does not need to know the definition of `A::aa`. **We only need to provide the size and alignment of a type in order to store it (as well as its name).** For example, if we knew that `A::aa` is 32 bytes in size, aligned to 8 bytes, then an importer of `B::foobar` has enough information to output assembly that calls the function. This is done by allocating space for `A::aa` on the stack, then passing a pointer to that space as a hidden parameter to `B::foobar`. This still creates a dependency that we need to recompile this module if `A::aa` exceeds 32 bytes in size or its alignment exceeds 8 bytes, but this is fundamentally unavoidable with unboxed types. As long as `A::aa` stays within its size bounds, an importer of `B::foobar` cannot see the fields of `A::aa` unless it imports `A::aa`, giving us exactly the symbol decoupling we want.

In our context of a compiler that compiles down to C code, how would we pass the right information to the underlying C compiler? A couple strategies come to mind:
1. We provide the entire type definition to the compiler, just like a `.h` file. However, code within a module is not allowed to access its fields unless it is imported. This is easier to implement since it functionally works like a header file, just with additional static code checks. However, this may cause us to crawl the codebase, as opposed to having symbols/modules truly be decoupled.
2. For each type, we generate a special variant where all fields are removed, and only size and alignment information remain. If a function declaration or type definition uses another type, it will actually use the special variant instead. This means that if a function returns a type, it will actually return the special variant instead. If the caller module also imported the underlying type, then they will receive conversion functions that transform between the special variant and the underlying type. This allows us to avoid unnecessary crawling the codebase as with option 1. However, this requires implementing functionality to collect and apply size/alignment information, which may not be possible.

Unfortunately, in the context of backwards compatibility with C code, option 2 is not possible. The first problem is that having the special variant and the underlying type share the same memory breaks strict aliasing rules in C99. While C code often does make use of this technique (also known as type-punning), and most mainstream C compilers will generate correct code, it is not guaranteed by the C spec and therefore not safe to rely on. The second problem is that the C spec does not control how values are passed to and returned from functions at the binary level - that is determined by the ABI of the platform the code is running on, which may decide to unpack small structs into registers. This essentially shuts down any hope of using this for our modules, unless we want to break backwards compatibility with existing C code. As such, we are stuck taking option 1 for our use-case.
